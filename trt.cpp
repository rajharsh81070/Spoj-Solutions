...
// Top Down Dp
ll calTotalVal(ll arr[], int i, int j, int day)
{
	if(i>j)
		return 0;
	...
	ll tmp1 = arr[i] + calTotalVal(arr, i+1, j, day+1);
	ll tmp2 = arr[j] + calTotalVal(arr, i, j-1, day+1);
...
int main(void)
{
	...
	ll res = calTotalVal(arr, 0, n-1, 1);
	f(i, 0, n){
		f(j, 0, n)
			pls(dp[i][j]);
		nl;
