...
int find_set(int arr_set[], int v){
	if(v==arr_set[v])
		return v;
	return arr_set[v] = find_set(arr_set, arr_set[v]);
}

void union_set(int arr_set[], int a, int b){
	a = find_set(arr_set, a);
	b = find_set(arr_set, b);
	if(a!=b)
		arr_set[b] = a;
}
	...
	w(t){
		...
		f(i, 0, k){
			...
			if(d==1){
				if(x>n||y>n)
					ans++;
				else if(find_set(arr_set, x)==find_set(arr_set, y))
					if(x!=y)
						ans++;
			}
			else{
